...
class GymEnvironment():
    ...
    def dataUnpack(self):
        obs_file = open('log/case5/obs.pkl', 'rb')
        self.obstacles = pickle.load(obs_file)
        obs_file.close()
        self.obs_visual()

        samples_file = open('log/case5/samples.pkl', 'rb')
        self.samples = pickle.load(samples_file)
        self.samples_tuple = pickle.load(samples_file)
        samples_file.close()

        neighbor_file = open('log/case5/neighbor.pkl', 'rb')
    ...
    def LazySP(self, weight=1):
        ...
        while True:
            ...
            edge_count += 1
            free_flag = self.stspace._edge_fp(np.array(e_select[0]), np.array(e_select[1]))
            collision_flag = not free_flag
            if collision_flag:
    ...
    def LazySP_Astar(self, start, goal, start_tuple, goal_tuple, weight=1):
